  ...
  return (
    <>
      <PageHeader
        ...
        description={t("adminCourses.desc")}
        actions={
          <>
            <Link href="/admin/import" className="btn btn-outline">
              <Settings2 size={15} /> {t("adminCourses.aiImport")}
            </Link>
            <button className="btn btn-primary">
              <Plus size={15} /> {t("adminCourses.newCourse")}
            </button>
          </>
        }
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <StatTile label={t("adminCourses.stat.published")} value={courses.length} />
        <StatTile label={t("adminCourses.stat.totalLessons")} value={totalAssigned} />
        <StatTile label={t("adminCourses.stat.fromBox")} value={courses.filter((c) => c.tags.includes("из коробки")).length || 4} />
        <StatTile label={t("adminCourses.stat.custom")} value={courses.filter((c) => c.category === "ERP и софт").length} />
      </div>
      ...
      <div className="card !p-3 mb-4 flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={t("adminCourses.search.placeholder")}
        ...
        <table className="w-full">
          <thead>
            <tr className="text-left text-[10.5px] uppercase tracking-[0.14em] font-semibold text-ink-mute border-b border-border bg-surface-alt/50">
              <th className="py-3 px-5">{t("adminCourses.col.course")}</th>
              <th className="py-3 px-3">{t("adminCourses.col.category")}</th>
              <th className="py-3 px-3">{t("adminCourses.col.lessons")}</th>
              <th className="py-3 px-3">{t("adminCourses.col.duration")}</th>
              <th className="py-3 px-3">{t("adminCourses.col.team")}</th>
              <th className="py-3 px-3">{t("adminCourses.col.assigned")}</th>
              <th className="py-3 px-3 w-10" />
            </tr>
          </thead>
          <tbody className="text-[13.5px]">
            {filtered.map((c) => {
              const title = locale === "uz" && c.titleUz ? c.titleUz : c.title;
              const cat = locale === "uz" && c.categoryUz ? c.categoryUz : c.category;
              const team = locale === "uz" && c.authorTeamUz ? c.authorTeamUz : c.authorTeam;
              return (
                <tr key={c.id} className="border-b border-border last:border-b-0 hover:bg-surface-alt/60 transition">
                  <td className="px-5 py-3.5">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      {c.required && <Chip tone="blue">{t("common.requiredShort")}</Chip>}
                  ...
                  <td className="px-3 py-3.5 text-ink-soft">{cat}</td>
                  <td className="px-3 py-3.5 tabular-nums">{c.totalLessons}</td>
                  <td className="px-3 py-3.5 tabular-nums">~{c.durationHours} {t("common.hoursShort")}</td>
                  <td className="px-3 py-3.5 text-[12.5px] text-ink-soft">{team}</td>
